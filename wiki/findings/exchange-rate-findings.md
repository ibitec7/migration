...
Results from the [Lead-Lag Analysis](analysis/lead-lag-analysis) of IMF Real Effective Exchange Rate (REER) against visa issuance data. Exchange rates provide a **meaningful but geographically limited** signal.
...
Months where month-over-month REER change exceeds the 80th percentile are flagged as **exchange rate shocks** (see [Surge Detection](analysis/surge-detection)). These shocks show elevated correlation with subsequent visa spikes in Dominican Republic and Mexico.
...
Despite limited coverage, exchange rates contribute to the [ensemble](models/horizon-aware-ensemble) through:
- Lag features in [Panel Construction](pipeline/panel-construction) (`exchange_rate_lag_1` through `exchange_rate_lag_6`)
...
- [Exchange Rates](data-sources/exchange-rates) — Source data details
- [Lead-Lag Analysis](analysis/lead-lag-analysis) — Statistical methodology
- [Event-Visa Findings](findings/event-visa-findings) — Complementary news-based findings
- [Model Performance](findings/model-performance) — Combined model results
- [Project Overview](project-overview) — Limitations discussion

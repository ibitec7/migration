...
`src/processing/build_panel.py` — see [Build Panel Detail](modules/build-panel-detail) for technical deep dive.

## Input Sources

| Source | Granularity | Module |
|--------|-------------|--------|
| [Visa Data](data-sources/visa-data) | Monthly issuances by country | `visa_master.parquet` |
| [Exchange Rates](data-sources/exchange-rates) | Monthly average REER | OECD data |
| [Google News](data-sources/google-news) | Event counts per month by country | `news_embeddings_labeled/` |
| [Google Trends](data-sources/google-trends) | Monthly keyword interest | Trend Parquets |
...
`data/processed/train_panel.parquet` — consumed by [Training Pipeline](pipeline/training-pipeline) and [Inference Pipeline](pipeline/inference-pipeline).

## Sequential Input Shape

The [model layer](modules/models-module) reshapes lag features into sequences:
...
- [Build Panel Detail](modules/build-panel-detail) — Technical deep dive (forward-fill, edge cases)
- [Data Processing](pipeline/data-processing) — Previous stage
- [Training Pipeline](pipeline/training-pipeline) — Next stage (model training)
- [Lead-Lag Analysis](analysis/lead-lag-analysis) — Statistical basis for the lag window choice
